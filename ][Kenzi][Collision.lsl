...
menu(key id)
{
    ...
    else if ((On == TRUE) && (ParticleOn = TRUE)){
        list main_menu = [ "■ On", "■ Particle", "Sounds", "Exit" ];
        llListenRemove(listener);
        channel = -1000000000 - (integer)(llFrand(1000000000));
        listener = llListen(channel, "", "", "");
    ...
    else if ((On == TRUE) && (ParticleOn = FALSE)){
        list main_menu = [ "■ On", "□ Particle", "Sounds", "Exit" ];
        llListenRemove(listener);
        channel = -1000000000 - (integer)(llFrand(1000000000));
        listener = llListen(channel, "", "", "");
...
init() 
{
    ...
    owner = llGetOwnerKey(llGetOwner());
...
default
{
    ...
    attach(key agent)
    {
        if (agent){
            if (On == TRUE){
                llOwnerSay("Collision are On..");
                CollisionOn();
            }
            else{
                llOwnerSay("Collision are Off..");
                CollisionOff();
            }
