...
list main_buttons       =   [ "Pussy rub", "Close" ];
...
list main_admin_buttons =   [ "Pussy rub", "Access", "Chatmode", "Close" ];
...
menu(key id)
{
    ...
    if ( id == llGetOwner())
        llDialog(id, (string)owner_name + "'s Pussy Menu\nWhat do you want to do with my pussy " + (string)name + "?", main_admin_buttons, chan);
    else
        llDialog(id, (string)owner_name + "'s Pussy Menu\nWhat do you want to do with my pussy " + (string)name + "?", main_buttons, chan);
}
...
rub(key id){
    ...
    if (id == llGetOwner()){
        if (_whisper)
            llWhisper(0, (string)owner_name + " rubs their own pussy!");
        else if (_say)
            llSay(0, (string)owner_name + " rubs their own pussy!");
        else if (_instant_message){
            llInstantMessage(llGetOwner(), "Hey " + (string)owner_name + " just rubbed your own pussy?!?");
        }
    }
    else{
        
        if (_whisper)
            llWhisper(0, (string)username + " rubs " + (string)owner_name + "'s pussy!");
        else if (_say)
            llSay(0, (string)username + " rubs " + (string)owner_name + "'s pussy!");
        else if (_instant_message){
            llInstantMessage(llGetOwner(), (string)username + " rubs your pussy!");
            llInstantMessage(id, "You rub " + (string)owner_name + "'s pussy!.");
...
default
{
    ...
    touch_start(integer total_number)
    {
        ...
        if (toucher_key == llGetOwner()){
            ...
            if (public){
                llSetObjectName("");
                llOwnerSay((string)username + " touched your pussy. (secondlife:///app/agent/" + (string)toucher_key + "/about)" );
                llSetObjectName(origName);
                //spank(toucher_key);
                menu(toucher_key);
            }
                ...
                llSetObjectName("");
                llOwnerSay("!DENIED! " + (string)username + " touched your pussy. (secondlife:///app/agent/" + (string)toucher_key + "/about)" );
                llSetObjectName(origName);
            }
        }
    }
