...
default {
    ...
    link_message( integer _sender, integer _num, string _message, key _id) {
        if ( _message == "KILJU_RESET" ) {
            llOwnerSay( "Resetting..." );
            llResetScript();
        } else if ( _message == "KILJU_AOON" ) {
            llSetTimerEvent( timerEventLength );
            animOverrideOn = TRUE;
            checkAndOverride();
        } else if ( _message == "KILJU_AOOFF" ) {
            llSetTimerEvent( 0 );
        ...
        } else if ( _message == "KILJU_NEXTSTAND" ) {
            doNextStand( TRUE );
        } else if ( llGetSubString(_message, 0, 14) == "KILJU_STANDTIME|" ) {
            // Stand time change
            standTime = (integer)llGetSubString(_message, 15, -1);
            llOwnerSay( "Stand cycle time: " + (string)standTime + " seconds" );
        
        } else if ( llGetSubString(_message, 0, 9) == "KILJU_LOAD|" ) {
