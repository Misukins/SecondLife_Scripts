...
string  tierName;
string  rentalGrade;
string  settings_file    = "Mecca Lounge Settings";
string  Context          = "Sit";
string  landmark;
string  security_orb;
...
dispData()
{
    list userName = llParseString2List(llGetDisplayName(renterID), [""], []);
    llSay(0, "=======================");
    llSay(0, "== Mecca Lounge Rental Space Information ==");
    llSay(0, "=======================");
...
menu(key id)
{
    if ( llrented == 0 & lllocked == 0 ){
        if ( id == llGetOwner() ){
            list main_menu = ["Lock", "Landmark", "Security", "Info", "Reset", "Exit"];
        ...
        if ( id == llGetOwner() ){
            list main_menu = ["Release", "Landmark", "Security", "Info", "Reset", "Exit"];
            string message = "Owner Options.  Select one of the options below...";
            llListenRemove(listener);
            channel = -1000000000 - (integer)(llFrand(1000000000));
            listener = llListen(channel, "", "", "");
            ...
            list main_menu = ["Release", "Info", "Landmark", "Security", "Exit"];
